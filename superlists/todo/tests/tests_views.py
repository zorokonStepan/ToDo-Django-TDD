...
class ListViewTest(TestCase):
    def test_uses_list_template(self):
        ...
        response = self.client.get(f"/todo/{mylist.id}/")
    ...
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post(f"/todo/{correct_list.id}/", data={"item_text": "A new item for an existing list"})
    ...
    def test_POST_redirects_to_list_view(self):
        ...
        response = self.client.post(f"/todo/{correct_list.id}/", data={"item_text": "A new item for an existing list"})
        self.assertRedirects(response, f"/todo/{correct_list.id}/")

    def test_validation_errors_end_up_on_lists_page(self):
        list_ = List.objects.create()
        response = self.client.post(f"/todo/{list_.id}/", data={"item_text": ""})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "todo/list.html")
        expected_error = escape("You can't have an empty list item")
        self.assertContains(response, expected_error)
